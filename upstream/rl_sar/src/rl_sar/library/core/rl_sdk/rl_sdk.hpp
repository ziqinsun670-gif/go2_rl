...
template <typename T> struct RobotCommand {
  struct MotorCommand {
    std::vector<int> mode;
    std::vector<T> q;
    std::vector<T> dq;
...
template <typename T> struct Observations {
...
public:
  ...
  void CSVLogger(const std::vector<float> &torque,
  ...
  // protect func
  std::atomic<bool> protection_stop_requested{false};
  std::atomic<bool> safe_stand_requested{false};
  std::atomic<bool> recovery_stand_low_gain_requested{false};
...
public:
  ...
  void ResetRLCommandFilter(const std::vector<float> &start_pos);
  void RLControl();

private:
  bool rl_command_filter_initialized_ = false;
  float rl_command_filter_elapsed_s_ = 0.0f;
  int rl_tracking_error_violation_count_ = 0;
  std::vector<float> rl_command_filter_entry_pos_;
  std::vector<float> rl_command_filter_last_pos_;
};
