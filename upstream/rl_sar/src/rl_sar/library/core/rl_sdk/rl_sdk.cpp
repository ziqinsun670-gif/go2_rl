...
void RL::StateController(const RobotState<float> *state,
  ...
  if (this->protection_stop_requested.load(std::memory_order_acquire)) {
    fsm.RequestStateChange("RLFSMStatePassive");
  }
  ...
  for (const std::string &observation :
    ...
    // ============= Other Observations =============
      ...
      if (this->motion_loader) {
        ...
        motion_cmd.insert(motion_cmd.end(), joint_pos_training.begin(),
                          joint_pos_training.end());
        motion_cmd.insert(motion_cmd.end(), joint_vel_training.begin(),
                          joint_vel_training.end());
      } else {
      ...
      if (this->motion_loader) {
        auto waist_sdk_indices =
            this->params.Get<std::vector<int>>("waist_joint_indices");
        ...
        std::vector<float> robot_torso_quat_w =
            MotionLoader::ComputeTorsoQuat(this->obs.base_quat, waist_angles);
        std::vector<float> ref_torso_quat_w =
            this->motion_loader->GetAnchorQuat();
        std::vector<float> init_quat = this->motion_loader->GetInitQuat();
        std::vector<float> motion_anchor_quat_w =
            QuaternionMultiply(init_quat, ref_torso_quat_w);
        std::vector<float> robot_quat_inv =
            QuaternionConjugate(robot_torso_quat_w);
        std::vector<float> relative_quat =
            QuaternionMultiply(robot_quat_inv, motion_anchor_quat_w);
      ...
      float motion_time = this->episode_length_buf *
...
void RL::TorqueProtect(const std::vector<float> &origin_output_dof_tau) {
  ...
  if (!out_of_range_indices.empty()) {
    ...
    this->protection_stop_requested.store(true, std::memory_order_release);
    std::cout << LOGGER::INFO
              << "Torque protection latched passive mode until restart."
              << std::endl;
  }
}
...
void RL::CSVLogger(const std::vector<float> &torque,
...
void RLFSMState::ResetRLCommandFilter(const std::vector<float> &start_pos) {
  rl_command_filter_initialized_ = !start_pos.empty();
  rl_command_filter_elapsed_s_ = 0.0f;
  rl_tracking_error_violation_count_ = 0;
  rl_command_filter_entry_pos_ = start_pos;
  rl_command_filter_last_pos_ = start_pos;
}
...
void RLFSMState::RLControl() {
  ...
  if (rl.output_dof_pos_queue.try_pop(_output_dof_pos) &&
    ...
    if (!rl_command_filter_initialized_ ||
        rl_command_filter_last_pos_.size() != _output_dof_pos.size()) {
      ResetRLCommandFilter(fsm_state->motor_state.q);
    }

    std::vector<float> filtered_dof_pos = _output_dof_pos;
    const float control_dt = rl.params.Get<float>("dt", 0.005f);
    const int decimation = std::max(1, rl.params.Get<int>("decimation", 1));
    const float policy_dt = control_dt * static_cast<float>(decimation);
    rl_command_filter_elapsed_s_ += policy_dt;

    const float transition_s =
        rl.params.Get<float>("real_rl_transition_seconds", 0.0f);
    if (transition_s > 0.0f &&
        rl_command_filter_entry_pos_.size() == filtered_dof_pos.size()) {
      const float alpha =
          std::min(1.0f, rl_command_filter_elapsed_s_ / transition_s);
      for (size_t i = 0; i < filtered_dof_pos.size(); ++i) {
        filtered_dof_pos[i] = (1.0f - alpha) * rl_command_filter_entry_pos_[i] +
                              alpha * filtered_dof_pos[i];
      }
    }

    const float max_slew_rate =
        rl.params.Get<float>("real_rl_target_slew_rate_rad_s", 0.0f);
    if (max_slew_rate > 0.0f &&
        rl_command_filter_last_pos_.size() == filtered_dof_pos.size()) {
      const float max_step = max_slew_rate * policy_dt;
      for (size_t i = 0; i < filtered_dof_pos.size(); ++i) {
        const float lo = rl_command_filter_last_pos_[i] - max_step;
        const float hi = rl_command_filter_last_pos_[i] + max_step;
        filtered_dof_pos[i] = std::min(std::max(filtered_dof_pos[i], lo), hi);
      }
    }

    const float tracking_threshold =
        rl.params.Get<float>("real_rl_tracking_error_guard_rad", 0.0f);
    if (tracking_threshold > 0.0f && fsm_state != nullptr &&
        !filtered_dof_pos.empty()) {
      const int required_frames =
          std::max(1, rl.params.Get<int>("real_rl_tracking_error_guard_frames", 3));
      const std::vector<std::string> joint_names = {
          "FR_hip", "FR_thigh", "FR_calf", "FL_hip", "FL_thigh", "FL_calf",
          "RR_hip", "RR_thigh", "RR_calf", "RL_hip", "RL_thigh", "RL_calf"};
      const int num =
          std::min({rl.params.Get<int>("num_of_dofs"),
                    static_cast<int>(filtered_dof_pos.size()),
                    static_cast<int>(fsm_state->motor_state.q.size())});

      float max_abs_error = 0.0f;
      float max_signed_error = 0.0f;
      int max_error_joint = -1;
      for (int i = 0; i < num; ++i) {
        const float error = filtered_dof_pos[i] - fsm_state->motor_state.q[i];
      ...
      if (max_abs_error > tracking_threshold) {
        rl_tracking_error_violation_count_ += 1;
      } else {
        rl_tracking_error_violation_count_ = 0;
      }

      if (rl_tracking_error_violation_count_ >= required_frames) {
        const std::string joint_name =
            max_error_joint >= 0 &&
                    max_error_joint < static_cast<int>(joint_names.size())
                ? joint_names[max_error_joint]
                : ("joint_" + std::to_string(max_error_joint));
        if (!rl.safe_stand_requested.exchange(true,
                                              std::memory_order_acq_rel)) {
          rl.recovery_stand_low_gain_requested.store(
          ...
          reason << "filtered lowcmd tracking error " << joint_name
                 << " q_cmd-q=" << max_signed_error << " rad exceeds "
                 << tracking_threshold << " rad for "
                 << rl_tracking_error_violation_count_
                 << " consecutive policy frames";
          std::cout << std::endl
                    << LOGGER::WARNING << "Real RL soft guard: "
                    << reason.str()
                    << ". Requesting default stand (RLFSMStateGetUp)."
                    << std::endl;
        }
        rl_tracking_error_violation_count_ = 0;
        return;
    ...
    for (int i = 0; i < rl.params.Get<int>("num_of_dofs"); ++i) {
      if (!filtered_dof_pos.empty()) {
        fsm_command->motor_command.q[i] = filtered_dof_pos[i];
      }
    ...
    if (!filtered_dof_pos.empty()) {
      rl_command_filter_last_pos_ = filtered_dof_pos;
    }
  }
}
