...
private:
  ...
  void RunModel();
  void RobotControl();
  bool CheckRealRLSafetyGuard();
  void RequestRealRLSafeStand(const std::string &reason);
  ...
  void InitRealTelemetryLog();
  void LogRealTelemetry(const RobotCommand<float> *command);
  ...
  std::ofstream real_telemetry_file;
  std::string real_telemetry_filename;
  std::mutex real_raw_policy_target_mutex;
